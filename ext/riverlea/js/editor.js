(function (CRM) {
  ...
  class CiviRiverleaStreamEditor extends HTMLElement {
    ...
    refreshPreview() {
      ...
      // [ML]FIXME framePreview.render(this.unsavedData);
    }
    ...
    renderInputs() {
      ...
      inputContainer.querySelector('.civi-riverlea-stream-color-inputs').append(
        ...
        createVariableInput(ts('Page Background'), colorSchemeVarsTarget, '--crm-c-page-bg', 'color'),
        createVariableInput(ts('Text'), colorSchemeVarsTarget, '--crm-text-color', 'color'),
        createVariableInput(ts('Primary background'), colorSchemeVarsTarget, '--crm-primary-color', 'color'),
